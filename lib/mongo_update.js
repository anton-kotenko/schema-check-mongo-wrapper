 ...
 */
var MongoQuery = require(__dirname + '/mongo_query.js'),
    /**
     ...
     */
    MongoUpdate = function (update, options) {
        this._update = update;
...
MongoUpdate.prototype = {
     ...
     * Function name is same as operator it represents
     * @see http://docs.mongodb.org/master/reference/operator/update/
    ...
    _operatorMethods: {
        ...
        $pull: function (field, fieldValue, document) {
            var fieldContainer = this._findElementContainer(field, document),
                fieldOwnName = this._getFieldOwnName(field),
                query;

            if (!fieldContainer || !(fieldContainer[fieldOwnName] instanceof Array)) {
                return document;
            }

            query = new MongoQuery({field: fieldValue});
            fieldContainer[fieldOwnName] = fieldContainer[fieldOwnName].filter(function (value) {
        ...
        $addToSet: function (field, fieldValue, document) {
            var fieldContainer = this._findElementContainer(field, document, true),
                fieldOwnName = this._getFieldOwnName(field),
                elementAlreadyExits = false,
                query;

            if (!(fieldContainer[fieldOwnName] instanceof Array)) {
                //short path, no need to check uniqueness
                fieldContainer[fieldOwnName] = [fieldValue];
                return document;
            }

            query = new MongoQuery({field: fieldValue});

            elementAlreadyExits = fieldContainer[fieldOwnName].some(function (value) {
                return query.match({field: value});
            });

            if (!elementAlreadyExits) {
                fieldContainer[fieldOwnName].push(fieldValue);
            }
